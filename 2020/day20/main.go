...
func main() {
	...
	log.Println("Part one:", cornerProduct(orientation))
	log.Println("Part two:", getWaterRoughness(renderImage(orientation)))
}
...
func (t *tile) Rotate(angle int) *tile {
	// log.Println("Rotating", t.id, angle)
...
func (t *tile) Flip(axis rune) *tile {
	// log.Printf("Flipping %d %c", t.id, axis)
	newData := make(map[int]map[int]rune)
	for x := 0; x <= t.xmax; x++ {
		newData[x] = make(map[int]rune)
	}
...
func findOrientation(tiles []*tile) map[int]map[int]*tile {
	...
	// If we use a grid twice as big as required then no matter which one we start with there will be room
	// for all the tiles
	for x := 0; x < size*2; x++ {
				...
				for i, b := range t.Borders() {
					...
					for idx, candidate := range tiles {
						for j, c := range candidate.Borders() {
							if r == c || b == c {
								// log.Println("Found place for", candidate.id)
								...
								switch i {
								case 0:
									// log.Println("Placed", candidate.id, x, y-1)
								...
								case 1:
									// log.Println("Placed", candidate.id, x+1, y)
									if b == c && r != c {
										candidate.Flip('y')
									}
...
func getWaterRoughness(t *tile) int {
	for a := 0; a < 2; a++ {
		for b := 0; b < 2; b++ {
			for i := 0; i < 3; i++ {
				if found := findSeaBois(t); found > 0 {
					return strings.Count(t.String(), "#") - (found * 15)
				}
				t = t.Rotate(90)
			}
			t.Flip('y')
...
func findSeaBois(t *tile) int {
	...
	nextTile:
		for y := 0; y < t.ymax-2; y++ {
			for sx := range seaBois.data {
				for sy := range seaBois.data[sx] {
